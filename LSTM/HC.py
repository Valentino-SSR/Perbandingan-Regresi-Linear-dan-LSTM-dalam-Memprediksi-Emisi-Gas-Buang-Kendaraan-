...
for i in range(len(X_future_scaled)):
    ...
    predicted_hc.append(y_pred[0][0])

# Simpan hasil
future_data['hc_predicted'] = predicted_hc

# Total HC
total_hc_predicted = future_data['hc_predicted'].sum()
print(f"\nTotal HC diprediksi selama 23 hari ke depan (LSTM): {total_hc_predicted:.2f} ppm")

# Plot hasil
plt.figure(figsize=(18, 5))
plt.plot(train_df['timestamp'], train_df['hc'], label="HC Aktual (7 Hari)", color='blue')
plt.plot(future_data['timestamp'], future_data['hc_predicted'], label="HC Prediksi LSTM (23 Hari)", color='green')
plt.xlabel("Waktu")
plt.ylabel("Kadar HC (ppm)")
plt.title("Prediksi LSTM Kadar HC Selama 23 Hari ke Depan + Data Aktual 7 Hari")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
