...
for i in range(len(X_future_scaled)):
    ...
    predicted_nh3.append(y_pred[0][0])

# Simpan hasil
future_data['nh3_predicted'] = predicted_nh3

# Total NH3
total_nh3_predicted = future_data['nh3_predicted'].sum()
print(f"\nTotal NH3 diprediksi selama 23 hari ke depan (LSTM): {total_nh3_predicted:.2f} ppm")

# Plot hasil
plt.figure(figsize=(18, 5))
plt.plot(train_df['timestamp'], train_df['nh3'], label="NH3 Aktual (7 Hari)", color='blue')
plt.plot(future_data['timestamp'], future_data['nh3_predicted'], label="NH3 Prediksi LSTM (23 Hari)", color='green')
plt.xlabel("Waktu")
plt.ylabel("Kadar NH3 (ppm)")
plt.title("Prediksi LSTM Kadar NH3 Selama 23 Hari ke Depan + Data Aktual 7 Hari")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
