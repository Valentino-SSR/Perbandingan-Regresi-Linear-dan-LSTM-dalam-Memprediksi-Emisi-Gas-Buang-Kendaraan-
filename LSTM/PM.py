...
target = 'pm'  # Ubah target dari 'co' menjadi 'pm'
...
for i in range(len(X_future_scaled)):
    ...
    predicted_pm.append(y_pred[0][0])

# Simpan hasil
future_data['pm_predicted'] = predicted_pm

# Total pm
total_pm_predicted = future_data['pm_predicted'].sum()
print(f"\nTotal pm diprediksi selama 23 hari ke depan (LSTM): {total_pm_predicted:.2f} ppm")

# Plot hasil
plt.figure(figsize=(18, 5))
plt.plot(train_df['timestamp'], train_df['pm'], label="pm Aktual (7 Hari)", color='blue')
plt.plot(future_data['timestamp'], future_data['pm_predicted'], label="pm Prediksi LSTM (23 Hari)", color='green')
plt.xlabel("Waktu")
plt.ylabel("Kadar pm (ppm)")
plt.title("Prediksi LSTM Kadar pm Selama 23 Hari ke Depan + Data Aktual 7 Hari")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
